...
def get_CDR_frag_dict():
    '''Call slide windown on each CDR entry in a directory
    Returns
    ------
    fragment_dict: dict
    ...
    fragment_dict = {"CDR_file": [], "resolution": [],
                     "sequence": [], "start_residue_idx": [],
                     "last_residue_index_from_end": []}

    CDR_files = os.listdir(CDR_FILE_PATH)
    pdb_id_lst = ut.dropDup([_[:PDB_ID_CHAR] for _ in CDR_files])
...
def slide_window(CDR_file=None, resolution=None):
    '''Performs sliding window on the passed in CDR entry
    Returns
    ------
    fragment_dict: dict
                   a dictionary containing
    ...
    try:
        ...
        residue_insertion_list_set = frut.get_residue_set_fromList(
            residue_insertion_list)
        num_residues = len(residue_insertion_list_set)

        # perform sliding window on atom data frame
        for start_residue_number in range(
            len(residue_insertion_list_set) - MIN_FRAG_LENGTH + 1
        ):
            ...
            for fragment_length in range(
                MIN_FRAG_LENGTH, num_residues - start_residue_number + 1
            ):

                last_residue = residue_insertion_list_set[
                    start_residue_number + fragment_length - 1
                ]
                last_residue_index_from_end = frut.find_max_index(
                    residue_insertion_list, last_residue)

                if last_residue_index_from_end == 0:
                    last_residue_index_from_end = -len(residue_insertion_list)

                fragment_df = atom_df.iloc[start_residue_idx:-
                                           last_residue_index_from_end]

                aa_list = fragment_df.drop_duplicates(
                    subset=["residue_number",
                            "insertion"]).residue_name.to_list()
                ...
                fragment_dict["last_residue_index_from_end"].append(
                    last_residue_index_from_end)
...
def main():
    ...
    for ind in result_df.index:
        ...
        pdb_to_save.df['ATOM'] = pd.merge(
            pdb_to_save.df['ATOM'], fragment_df, how='inner')
        pdb_save_file_name = os.path.join(ut.checkDir(CDR_FRAGMENT_PATH),
                                          file[:-PDB_ID_CHAR] +
                                          '_frag_'+residue_list[0] +
                                          '_'+residue_list[-1] + '.pdb')
        pdb_to_save.to_pdb(path=pdb_save_file_name,
