...
class TestStrucUt(unittest.TestCase):
    def setUp(self):
        ...
        self.crop_struct[0][self.rand_chain].detach_child(self.rand_res.id)
        self.d3to1 = {'CYS': 'C', 'ASP': 'D', 'SER': 'S', 'GLN': 'Q', 'LYS': 'K',
                      'ILE': 'I', 'PRO': 'P', 'THR': 'T', 'PHE': 'F', 'ASN': 'N',
                      'GLY': 'G', 'HIS': 'H', 'LEU': 'L', 'ARG': 'R', 'TRP': 'W',
                      'ALA': 'A', 'VAL': 'V', 'GLU': 'E', 'TYR': 'Y', 'MET': 'M'}
        sr.compute(self.control_struct[0], level="R")
        sr.compute(self.crop_struct[0], level="R")
        self.no_sasa_struct = parser.get_structure(self.test_pdb_id, self.pdb_file_name)
        self.agsasa_list = list()
        for chain in self.control_struct[0]:
            for res in chain:
                if res != self.rand_res:
                    sasa_val_full = round(res.sasa, 2)
                    sasa_val_crop = round(self.crop_struct[0][chain.id][res.id].sasa, 2)
                    if sasa_val_crop >= sasa_val_full+0.4:
                        if res.resname in self.d3to1.keys():
                            self.agsasa_list.append(f"{res.id[1]}|{chain.id}|{res.resname}")


    def test_get_structurefrompdb(self):
        structure = stut.get_structurefrompdb(self.test_pdb_id)
        self.assertEqual(list(structure.get_residues()), list(self.control_struct.get_residues()))
    ...
    def test_find_SASAcontact(self):
        agsasa_list = stut.find_SASAcontact(self.control_struct, self.crop_struct)
        agsasa_empty_list = stut.find_SASAcontact(self.no_sasa_struct, self.crop_struct)
